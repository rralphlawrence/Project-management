...
function queryTable($db, $deanName, $semester, $sy){
        ...
        $sqlforNoAccount = "SELECT Course, Author, File, School_Year, Semester, Date_Scheduled, Date_Submitted, Status FROM tblfiles WHERE Dean = '$deanName' AND Semester='$semester' AND School_Year='$sy' ORDER BY Author ASC, Course ASC, File ASC";
        $stmt = $db->query( $sqlforNoAccount);

        while ($row = $stmt->fetch(PDO::FETCH_OBJ)) {
                $this->Cell(8);
                $this->Cell(30,10,$row->Course,1,0,'L');
                $this->Cell(70,10,$row->Author,1,0,'L');
...
function Signatory($deanName,$MrorMs)
{
    ...
    $this->Cell(15);
    $this->SetFont('Arial','B',12);
    $this->Cell(30,10,'College Dean',0,0,'C');
    // Line break
    $this->Ln(20);
}
