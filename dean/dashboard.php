    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            <div class="user-naming" id="naming">
                <i id="icon" class='far fa-user-circle nav__icon'></i>
        ...
        <main>
                <div class="cards">
                    ...
                    <div class="card-2 card">
                        <div class="count">
                        <?php
                            $query = "SELECT COUNT(File) AS Total_Files FROM tblfiles WHERE  Dean = '$nameHolder' AND (Status = 'ON TIME' OR Status = 'OVERDUE')";    
                    ...
                    <div class="card-3 card">
                        <div class="count">
                        <?php
                            $query = "SELECT COUNT(Status) AS Files_Ontime FROM tblfiles WHERE  Dean = '$nameHolder' AND Status = 'ON TIME'";    
                            $stmt = $conn->prepare($query);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $row = $result->fetch_assoc();
                    ...
                    <div class="card-4 card">
                        <div class="count">
                        <?php
                            $query = "SELECT COUNT(Status) AS Files_Overdue FROM tblfiles WHERE  Dean = '$nameHolder' AND Status = 'OVERDUE'";    
                            $stmt = $conn->prepare($query);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $row = $result->fetch_assoc();
                    ...
                    <div class="card-5 card">
                        ...
                        <div class="count">
                            <h2><?php echo date("d")?><span><?php echo " "?> </span><span><i class="fas fa-calendar-alt"></i></span></h2>
                        </div>
                        <div class="title">
                    ...
                    <div class="card-6 card">
                        <div class="count">
                        <?php
                            $query = "SELECT COUNT( DISTINCT  Fullname) AS Total_Faculty FROM tblfaculty WHERE  Dean = '$nameHolder'";    
                            $stmt = $conn->prepare($query);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $row = $result->fetch_assoc();
                        ?>  
                            <h2> <?= $row['Total_Faculty']; ?><span><?php echo " "?> </span><span><i class="bx bxs-face"></i></span></h2>
                        </div>
                        <div class="title">
                    ...
                    <div class="card-7 card">
                        <div class="count">
                        <?php
                            $query = "SELECT COUNT(Status) AS Files_Inprogress FROM tblfiles WHERE  Dean = '$nameHolder' AND Status = 'IN PROGRESS'";    
                            $stmt = $conn->prepare($query);
                            $stmt->execute();
                            $result = $stmt->get_result();
                            $row = $result->fetch_assoc();
